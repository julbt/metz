...
app.get('/api/orders/:id/fulfillment_orders', async (req, res) => {
    try {
        const { id } = req.params;
        const data = await shopifyAdminRequest(`/orders/${id}/fulfillment_orders.json`);
        res.json(data);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
// Create fulfillment (marks order as fulfilled + triggers Shopify confirmation email to customer)
...
app.put('/api/boutique-config', (req, res) => {
    try {
        ...
        if (updates.collectionsOrder !== undefined) {
            config.collectionsOrder = updates.collectionsOrder;
        }
